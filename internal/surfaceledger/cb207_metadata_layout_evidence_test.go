...
type cb207MetadataLayoutComparison struct {
	Candidate struct {
...
func TestCB207MetadataLayoutRowsHaveExactFixtureAndOracleEvidence(t *testing.T) {
	toolsRoot := filepath.Join("..", "..")
	comparisonPath := filepath.Join(toolsRoot, "docs", "fixtures", "salesforce-current-base-cb207-metadata-layout-deterministic-api67-comparisons.json")
	var comparison cb207MetadataLayoutComparison
	readJSON(t, comparisonPath, &comparison)
	if comparison.Candidate.Commit != "6d3aa2da1838f54832577aa7ca1fdb86d0b7fc22" || comparison.Candidate.SHA256 != comparison.Local.CandidateSHA256 || comparison.Profile.SelectedRowCount != 28 || comparison.Profile.PredecessorNonDeferredGaps != 4406 || comparison.Profile.ExpectedSuccessorGaps != 4378 || len(comparison.LocalFixtures) != 1 || len(comparison.Comparisons) != 1 {
		t.Fatalf("CB207 provenance = %#v", comparison)
	}
	...
	if len(wantIDs) != 28 || comparison.Comparisons[0].CaseID != "metadata-layout-deterministic-api67" || comparison.Comparisons[0].Status != "pass" {
		t.Fatalf("CB207 IDs/comparison = %d %#v", len(wantIDs), comparison.Comparisons)
	}
	...
	if local.Status != "passed" || local.ExitCode != 0 {
		t.Fatalf("CB207 local report = %#v", local)
	}
	var sf struct {
	...
	if sf.Status != 0 || !sf.Result.Success || !sf.Result.Compiled {
		t.Fatalf("CB207 Salesforce report = %#v", sf)
	}
	var profile struct {
	...
	if len(profile.NonDeferredGaps) != comparison.Profile.PredecessorNonDeferredGaps {
		t.Fatalf("CB207 predecessor profile gaps = %d", len(profile.NonDeferredGaps))
	}
}
