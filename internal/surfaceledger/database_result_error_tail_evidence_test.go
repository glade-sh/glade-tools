	...
	tests := []struct {
	...
	}{
		{
			...
			witness: []string{
				...
				"error.getExtendedErrorDetails()", "error.getFields()", "error.getMessage()", "error.getStatusCode()",
				"Database.UndeleteResult restored = Database.undelete(record, false);",
				"Database.UndeleteResult restoredErrors = (Database.UndeleteResult)JSON.deserialize",
				"System.assertNotEquals(null, restoredErrors.getErrors());",
				"System.assertEquals(0, restoredErrors.getErrors().size());",
