...
func TestPrivateFlowInterviewTypeHasDirectLocalEvidence(t *testing.T) {
	const (
		id    = "apex:Flow.Interview"
		owner = "private-corpus-flow-interview-type"
	)
	...
	if fixture.Command.Kind != "test" || len(fixture.Source) != 1 || !strings.Contains(fixture.Source[0].Content, "Flow.Interview interview = Flow.Interview.createInterview(") {
		t.Fatalf("fixture lacks direct Flow.Interview witness: %#v", fixture)
	}
