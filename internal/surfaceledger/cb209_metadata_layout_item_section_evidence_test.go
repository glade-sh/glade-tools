...
type cb209MetadataLayoutItemSectionComparison struct {
	Candidate struct {
...
func TestCB209MetadataLayoutItemSectionRowsHaveExactFixtureAndOracleEvidence(t *testing.T) {
	toolsRoot := filepath.Join("..", "..")
	comparisonPath := filepath.Join(toolsRoot, "docs", "fixtures", "salesforce-current-base-cb209-metadata-layout-item-section-deterministic-api67-comparisons.json")
	var comparison cb209MetadataLayoutItemSectionComparison
	readJSON(t, comparisonPath, &comparison)
	if comparison.Candidate.Commit != "6d3aa2da1838f54832577aa7ca1fdb86d0b7fc22" || comparison.Candidate.SHA256 != comparison.Local.CandidateSHA256 || comparison.Profile.SelectedRowCount != 24 || comparison.Profile.PredecessorNonDeferredGaps != 4406 || comparison.Profile.ExpectedSuccessorGaps != 4382 || len(comparison.LocalFixtures) != 1 || len(comparison.Comparisons) != 1 {
		t.Fatalf("CB209 provenance = %#v", comparison)
	}
	...
	if len(wantIDs) != 24 || comparison.Comparisons[0].CaseID != "metadata-layout-item-section-deterministic-api67" || comparison.Comparisons[0].Status != "pass" {
		t.Fatalf("CB209 IDs/comparison = %d %#v", len(wantIDs), comparison.Comparisons)
	}
	...
	if local.Status != "passed" || local.ExitCode != 0 {
		t.Fatalf("CB209 local report = %#v", local)
	}
	var sf struct {
	...
	if sf.Status != 0 || !sf.Result.Success || !sf.Result.Compiled {
		t.Fatalf("CB209 Salesforce report = %#v", sf)
	}
	var profile struct {
	...
	if len(profile.NonDeferredGaps) != comparison.Profile.PredecessorNonDeferredGaps {
		t.Fatalf("CB209 predecessor profile gaps = %d", len(profile.NonDeferredGaps))
	}
}
