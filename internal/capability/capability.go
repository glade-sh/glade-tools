...
var mvpFeatures = []Feature{
	...
	{ID: "lsp.context-completion", Area: "Developer experience", Name: "Context-aware Apex completion", Status: StatusPartial, Required: false, Notes: "Post-MVP work covers deeper context-aware completion ranking and insertion beyond local MVP type, member, SObject, field, and keyword completion; the first test-backed context ranks SObject fields before top-level names inside SOQL SELECT projection clauses."},
	{ID: "profile.pprof-and-timing", Area: "Developer experience", Name: "pprof output and wall-clock statement timing", Status: StatusPartial, Required: false, Notes: "Post-MVP work now has test-backed native trace wall-clock summary attribution in JSON/Markdown profile reports; remaining work covers pprof-compatible CPU output and deeper per-statement wall-clock attribution beyond native trace/profile reports."},
