...
var mvpFeatures = []Feature{
	...
	{ID: "dap.live-ide-orchestration", Area: "Developer experience", Name: "Richer live IDE debug orchestration", Status: StatusSupported, Required: false, Notes: "VS Code launch aliases, debug profile wiring, and local glade exec/test DAP sessions are covered by deterministic local tests."},
	{ID: "lsp.context-completion", Area: "Developer experience", Name: "Context-aware Apex completion", Status: StatusSupported, Required: false, Notes: "Context-aware completions include SOQL SELECT projection ranking that prefers SObject fields before top-level names, with focused LSP tests."},
	{ID: "profile.pprof-and-timing", Area: "Developer experience", Name: "pprof output and wall-clock statement timing", Status: StatusSupported, Required: false, Notes: "Profile reports include pprof-compatible output plus wall-clock statement timing summaries in JSON/Markdown native trace reports."},
	{ID: "watch.profile-trace-reports", Area: "Developer experience", Name: "Profile-driven watch reports", Status: StatusSupported, Required: false, Notes: "Watch runs emit profile and trace report events beside affected-test selection, rerun, and stale-run suppression events."},
	{ID: "limits.configurable-local-profiles", Area: "Limits", Name: "Configurable local governor cap profiles", Status: StatusSupported, Required: false, Notes: "glade exec, test, and server accept deterministic local limit profiles and per-counter caps for supported governor counters."},
	{ID: "limits.exact-salesforce-accounting", Area: "Limits", Name: "Exact hosted Salesforce governor accounting", Status: StatusUnsupported, Required: false, Notes: "Exact Salesforce runtime counter deltas require hosted execution accounting and remain outside deterministic local execution."},
	{ID: "server.rest-breadth.local-expanded", Area: "Local API server", Name: "Expanded local REST, Composite, Bulk, layout, and metadata jobs", Status: StatusSupported, Required: false, Notes: "Composite Batch, Tree, and Composite Graph local requests, Bulk API v2 simple query jobs, layout/default-value metadata, metadata job status, REST limits, and local Tooling shapes are covered by server tests."},
	{ID: "server.rest-breadth.hosted-auth-live-org-deploy", Area: "Local API server", Name: "Hosted auth, live-org Tooling, and deployment services", Status: StatusUnsupported, Required: false, Notes: "OAuth token issuance, live org-only Tooling objects, live metadata deploy/retrieve, and broader hosted REST namespaces require Salesforce services."},
	{ID: "compat.fixture-expansion", Area: "Release", Name: "Compatibility fixture expansion", Status: StatusSupported, Required: false, Notes: "Fixture evidence coverage is gated by the first-party compat tooling and generated fixture reports."},
	{ID: "release.distribution-automation", Area: "Release", Name: "Package-manager distribution, signing, and promotion automation", Status: StatusSupported, Required: false, Notes: "Release scripts build installable archives, checksums, manifests, smoke checks, and promotion metadata for the local binary distribution path."},
	{ID: "stdlib.platform-breadth", Area: "Runtime", Name: "Broader System stdlib and platform service parity", Status: StatusSupported, Required: false, Notes: "All locally pursuable stdlib rows are modeled as supported local behavior or split into exact hosted-service unsupported rows with stable diagnostics."},
}
