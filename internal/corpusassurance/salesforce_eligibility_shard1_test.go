...
func TestSalesforceEligibilityShard1FixturesAreExplicit(t *testing.T) {
	tests := []struct {
		...
		{"core-messaging-sendemail-error-fields-runtime.json", "policy-local-only", "All owned rows are governed by compile-only or deterministic-mock action authority; this local exec fixture is policy-local-only and grants zero Salesforce parity credit.", false},
		...
		{"core-runtime-metadata-deploy-dto-api67.json", "policy-local-only", "All owned rows are governed by compile-only or deterministic-mock action authority; this local exec fixture is policy-local-only and grants zero Salesforce parity credit.", false},
