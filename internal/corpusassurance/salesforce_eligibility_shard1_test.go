...
func TestSalesforceEligibilityShard1FixturesAreExplicit(t *testing.T) {
	tests := []struct {
		...
		{"core-runtime-exception-accessors.json", "policy-local-only", "The command asserts Glade-local exception line numbers and empty stack traces for constructed AssertException and AsyncException values; this fixture is policy-local-only and grants zero Salesforce parity credit.", false},
