...
func validateOrchestratorSSHRawFetchRequest(request OrchestratorSSHRawFetchRequest) (string, string, string, error) {
	...
	for _, path := range []string{request.WorkerBin, request.PlanPath, request.RemotePlanPath, request.RemoteScopePath, request.LeasePath, request.DispatchPath, request.BundlePath, request.SFBin, request.RemoteRoot, request.LocalRoot} {
	...
	commandRequest := OrchestratorSSHDispatchRequest{Host: request.Host, WorkerBin: request.WorkerBin, PlanPath: request.PlanPath, RemotePlanPath: request.RemotePlanPath, RemoteScopePath: request.RemoteScopePath, LeasePath: request.LeasePath, BundlePath: request.BundlePath, TargetOrg: request.TargetOrg, SFBin: request.SFBin, OutputRoot: request.RemoteRoot}
	command := orchestratorSSHWorkerOnceCommand(commandRequest, request.Plan.Definition.Tools.SHA256, request.Plan.Definition.ControlledInputSHA256[OrchestratorToolsAMD64Input], planSHA, leaseSHA, request.Plan.Definition.ScopeSHA256, request.DevHub)
