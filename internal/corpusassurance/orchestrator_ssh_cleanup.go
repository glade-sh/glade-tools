...
func validateOrchestratorSSHCleanupTakeoverRequest(request OrchestratorSSHCleanupTakeoverRequest, timeout time.Duration) (OrchestratorCampaignPlan, []byte, OrchestratorLease, []byte, OrchestratorSSHDispatchReceipt, []byte, error) {
	...
	original := OrchestratorSSHDispatchRequest{Host: request.Host, WorkerBin: request.WorkerBin, PlanPath: request.RemotePlanPath, LeasePath: request.RemoteLeasePath, BundlePath: request.RemoteBundlePath, TargetOrg: request.Claim.AllocationAlias, SFBin: request.RemoteSFBin, OutputRoot: request.RemoteRoot}
