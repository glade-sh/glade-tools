...
func TestValidateRemoteAttemptTarget(t *testing.T) {
	validHash := strings.Repeat("a", 64)
	validParent := "/private/tmp/glade-assurance-worker"
	validRoot := filepath.Join(validParent, "assurance-"+validHash[:16]+"-run")
	if err := validateRemoteAttemptTarget(validHash, "salesforce-worker", "operator@salesforce-worker", validParent, validRoot); err != nil {
		t.Fatalf("valid remote target rejected: %v", err)
	}
	for _, target := range []struct {
		...
		{name: "unsafe host", host: "operator@host;rm", root: validRoot},
		{name: "non-child root", host: "operator@salesforce-worker", root: "/private/tmp/glade-assurance-other/assurance-" + validHash[:16] + "-run"},
	} {
		t.Run(target.name, func(t *testing.T) {
			if err := validateRemoteAttemptTarget(validHash, "salesforce-worker", target.host, validParent, target.root); err == nil {
				t.Fatalf("accepted invalid remote target: %#v", target)
			}
		})
	}
}
...
func TestPreserveRemoteFailureRejectsChecksumDryRunDifferences(t *testing.T) {
	root := t.TempDir()
	bindingPath := filepath.Join(root, "SALESFORCE_REMOTE_CLEANUP_AUTHORITY.json")
	writeRemoteCleanupAuthority(t, bindingPath, "operator@salesforce-worker", filepath.Join(remoteCleanupParent, "dry-run-difference"))
	handoff := filepath.Join(root, "HANDOFF.md")
	if err := os.WriteFile(handoff, []byte("Current gate: Salesforce lane\nLast completed command: start\nNext command: run\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	_, err := PreserveRemoteFailure(RemoteFailurePreserveRequest{
		AttemptPath: bindingPath + ".attempt", BindingPath: bindingPath, Phase: "salesforce-lane", PhaseExit: 1, HandoffPath: handoff, OutputPath: filepath.Join(root, "failure"),
		runner: func(_ context.Context, _, destination, _ string, checksum bool) (salesforceCommandOutput, error) {
			if checksum {
				return salesforceCommandOutput{ExitCode: 0, Stdout: []byte("changed.json\n")}, nil
			}
			if err := os.WriteFile(filepath.Join(destination, "partial.json"), []byte("retained"), 0o600); err != nil {
				t.Fatal(err)
			}
	...
	if err == nil {
		t.Fatal("PreserveRemoteFailure accepted checksum dry-run differences")
	}
}
