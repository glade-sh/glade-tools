...
func runCorpusAssurance(ctx context.Context, args []string, w io.Writer) error {
	...
	case "usage":
		...
		if err := requiredAssuranceFlags(*inventory, *ledger, *manifest, *profile, *decisions, *output); err != nil {
			return err
		}
		usage, err := corpusassurance.BuildSealedCorpusUsage(*inventory, *ledger, *manifest, *profile, *decisions, *output)
		if err != nil {
			return err
		}
	...
	case "oracle-plan":
		...
		plan, err := corpusassurance.PlanOracleFromFiles(*profileOutput, *usage, *proof, *directives, *output)
		if err != nil {
			return err
		}
...
Usage:
  ...
  glade-tools corpus assurance usage --inventory-spec <IN_SCOPE.json> --ledger <ledger.json> --manifest <MANIFEST.json> --profile <source-profile.json> --decisions <USAGE_DECISIONS.json> --output <CORPUS_USAGE.json>
