...
func runCorpusAssurance(ctx context.Context, args []string, w io.Writer) error {
	...
	case "oracle-bundle":
		...
		release, proof, fixtures := flags.String("release-validation", "", ""), flags.String("local-proof", "", ""), flags.String("fixture-manifest", "", "")
		...
		if err := requiredAssuranceFlags(*profile, *plan, *authority, *release, *proof, *fixtures, *filter, *scratch, *tools, *output); err != nil {
			return err
		}
		bundle, err := corpusassurance.BuildOracleBundle(corpusassurance.OracleBundleRequest{ProfilePath: *profile, PlanPath: *plan, AuthorityPath: *authority, ReleaseValidationPath: *release, LocalProofPath: *proof, FixtureManifestPath: *fixtures, FilterScriptPath: *filter, ScratchDefinitionPath: *scratch, ToolsAMD64Path: *tools, OutputPath: *output})
		if err != nil {
			return err
		}
