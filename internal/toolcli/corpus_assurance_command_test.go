...
func TestCorpusAssuranceSurfaceOraclePlanRejectsPositionalSurfaceIDs(t *testing.T) {
	args := []string{
		"corpus", "assurance", "surface-oracle-plan", "--wave-plan", "/wave.json", "--scope", "/scope.json",
		"--profile", "/profile.json", "--local-proof", "/proof.json", "--fixture-manifest", "/fixtures.json",
		"--coverage", "/coverage.json", "--output", "/oracle", "apex:System.run",
	}
	var stdout, stderr bytes.Buffer
	if code := Run(context.Background(), args, &stdout, &stderr); code == 0 || !strings.Contains(stderr.String(), "does not accept positional SurfaceIDs") {
		t.Fatalf("positional SurfaceID accepted: code=%d stdout=%q stderr=%q", code, stdout.String(), stderr.String())
