...
func compatUsage() string {
	parts := []string{
		...
		"corpus check --root <corpus-root> --glade <binary> --out <dir> [--expected-diagnostics <path>] [--simulate-source-api-version <MAJOR.0>] [--fail-on-unclassified] [--max-unclassified <n>] [--fail-on-check-closure]",
...
func runCompatCorpus(ctx context.Context, args []string, w io.Writer) error {
	if len(args) == 0 || isHelpArg(args[0]) {
		fmt.Fprintln(w, "usage: glade-tools corpus check --root <corpus-root> --glade <binary> --out <dir> [--expected-diagnostics <path>] [--simulate-source-api-version <MAJOR.0>] [--fail-on-unclassified] [--max-unclassified <n>] [--fail-on-check-closure]")
		return nil
	}
	if args[0] != "check" {
		return errors.New("usage: glade-tools corpus check --root <corpus-root> --glade <binary> --out <dir> [--expected-diagnostics <path>] [--simulate-source-api-version <MAJOR.0>] [--fail-on-unclassified] [--max-unclassified <n>] [--fail-on-check-closure]")
	}
	...
	for i := 1; i < len(args); i++ {
		...
		case "--expected-diagnostics":
			i++
			value, err := argValue(args, i, "--expected-diagnostics")
			if err != nil {
				return err
			}
			options.ExpectedDiagnostics = value
...
func runCompatLwcCorpus(args []string, w io.Writer) error {
	...
	if strings.TrimSpace(options.Root) == "" {
		return errors.New("usage: glade-plugin-compat lwc corpus --root <path> [--out <path>] [--json] [--check] [--include-repos <a,b>]")
	}
	...
	if jsonOut {
		if err := compat.WriteLwcCorpusJSON(w, report); err != nil {
			return err
		}
	} else {
	...
	if options.Check {
		return compat.CheckLwcCorpusReport(report)
	}
	return nil
}
...
Targets:
  ...
  package-phase1-base-components, visualforce-base-components,
